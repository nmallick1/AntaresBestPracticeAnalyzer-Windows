...
#region Code for your detector goes here
    $slotsInfoUrl = "/subscriptions/" + $subscriptionId  + "/resourceGroups/" + $resourceGroup + "/providers/Microsoft.Web/sites/" + $resourceName + "/slots/?api-version=2016-08-01" 

    $slotsInfoJSON = ARMClient.exe get $slotsInfoUrl
    $slotsInfo = $slotsInfoJSON | ConvertFrom-Json


    If($slotsInfo.value.Count -gt 0){
        #There is a slot present for this site.
        $adheringToBestPractice = $true
    }
    else
    {
        #There are no slots present for this site so the code is being deployed against the production slot most likely impacting production in someway or the other
